...
def agent(df, openai_api_key, query):
    openai_agent = create_pandas_dataframe_agent(OpenAI(temperature=0, api_key=openai_api_key), df, verbose=True)
    prompt = (
        """
...
# prompt = input(">>")
# agent("test.csv",openai_api_key, prompt)